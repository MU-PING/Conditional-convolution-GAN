...
from tensorflow.keras.layers import Dense, Reshape, Flatten, ZeroPadding2D, Concatenate
from tensorflow.keras.layers import LeakyReLU, Input, UpSampling2D, Conv2D, Activation, Dropout, Embedding
...
class GAN():
    ...
    def build_generator(self):
        ...
        model.add(BatchNormalization(momentum=0.001))
        model.add(Activation("relu"))
        ...
        label_embedding = Flatten()(Embedding(self.num_classes, self.embedding_dim)(label))
        model_input = Concatenate()([noise, label_embedding])
        fakeImage = model(model_input)
        
        model = Model([noise, label], fakeImage)
        model.summary()
    ...
    def build_discriminator(self):
        ...
        fakeImage = Input(shape=self.img_shape)
        flat_fakeImage = model(fakeImage)
        
        label = Input(shape=(1,), dtype='int32')
        label_embedding = Flatten()(Embedding(self.num_classes, self.embedding_dim)(label))
        
        x = Concatenate()([flat_fakeImage, label_embedding])
        x = Dense(128)(x)
        ...
        model = Model([fakeImage, label], validity)
        model.summary()
